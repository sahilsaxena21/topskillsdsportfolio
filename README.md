# Programmatic Segmentation of Skill Needs in Canada's Data Science Industry
...
In a real-world setting, one of ways project success can be measured is by receiving direct feedback from our target users. For example, we can engage a random sample of our target population and ask them to rate the effectiveness of the app (via. a quantitative survey). This sample can also be engaged to collect feedback on how the app can be made better (via. a qualitative survey), to advance iterative and incremental improvement.
...
**Interpreting Segments for Job Title ‘Data Scientist’ [1, 2, 3]**


As per the above network graph, current industry needs can largely be seperated into 3 types of data scientists. The segment profiles are described as below. The trait characteristics of each cluster are **bolded**.
