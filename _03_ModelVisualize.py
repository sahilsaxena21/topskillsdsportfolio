...
from _02_FeatureExtractorProcessor import Feature_Extractor_and_Processor
...
class Modeling_and_Visualization:
    ...
    def pmiCal(self, df_temp, x, label_col):
        pmilist=[]
    ...
    def pmiIndivCal(self, df_temp,x,gt, label_col):
    ...
    def pmiForAllCal(self, df_temp, topk, label_col):
        ...
        for word in tqdm(topk):
            pmilist.append([word[0]]+[self.pmiCal(df_temp,word[0], label_col)])

            for label in df_temp[label_col].value_counts().index.tolist():
                pmi_label_dict[label].append([word[0]]+[self.pmiIndivCal(df_temp,word[0],label,label_col)])
    ...
    def get_pmi_dict(self, df, topk, label_col):
        pmi_dict, pmidf = self.pmiForAllCal(df, topk, label_col)
        return pmi_dict
    ...
    def _add_cluster_label_each_title(self, df_tools, job_title, job_group, number_of_clusters, job_description_col, url_col):
        pd.options.mode.chained_assignment = None

        #setup only data science df
        df_ds = df_tools[df_tools[job_group] == job_title]
    ...
    def _get_high_pmi_words_each_cluster(self, df_label_dict, topk_tf_idf_single, topk_tf_idf_phrase,                           df_ds_subset, job_description_col):
        ...
        pmi_dict_single = {}
        pmi_dict_phrase = {}
        pmi_dict_single, pmidf_single = self.pmiForAllCal(df_ds_subset_single, topk_tf_idf_single, "cluster_label")
        pmi_dict_phrase, pmidf_phrase = self.pmiForAllCal(df_ds_subset_phrase, topk_tf_idf_phrase, "cluster_label")
    ...
    def calculate_chi2(self, word_1_count,word_2_count,cocc_count, number_of_records):
    ...
    def add_edges(self, df_tools, clean_job_title_col, url_col, job_title_col,                      cluster_label_col, job_description_col):    
        ...
        for max_clusters, dict_items in self.topwords_by_title_dict.items():
            ...
            for job_title, nodes_list in dict_items.items():
                ...
                df_edge['p_value_chi2'] = df_edge.apply(lambda row : self.calculate_chi2(row['word_1_count'], 
...
#                 self.net_dict[max_clusters][job_title].hrepulsion()
    ...
    def generate_network_graph(self, df_tools, label_col, url_col, job_title_col, encoded_job_title_col, description_col):
        self.add_nodes()
        self.add_edges(df_tools, label_col, url_col, job_title_col, encoded_job_title_col, description_col)
